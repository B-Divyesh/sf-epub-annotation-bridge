...
const titles: Record<string, string> = {
  ...
  "/demo": "Demo — EPUB Annotation Bridge",
  "/ledger": "Ledger — EPUB Annotation Bridge",
  "/download": "Download — EPUB Annotation Bridge",
  "/privacy": "Privacy — EPUB Annotation Bridge",
  "/terms": "Terms — EPUB Annotation Bridge",
  "/404": "Not found — EPUB Annotation Bridge",
};
...
const stamp = (text: string, kind = "") =>
  `<span class="stamp ${kind}">${escapeHtml(text)}</span>`;
const checkoutPending = () =>
  `<div class="checkout-pending" role="status"><span class="button primary" aria-disabled="true">Checkout is being registered</span><p>Folder watching remains a $19 one-time desktop license. Sociobot must register checkout before it can open.</p></div>`;

function annotationRows(rows: Annotation[], limit?: number): string {
  const shown = typeof limit === "number" ? rows.slice(0, limit) : rows;
  return shown
    .map(
      (row) => `<article class="ledger-row" data-id="${escapeHtml(row.id)}">
...
function landing(): string {
  return `${header()}<main id="main" tabindex="-1">
    ...
    <section class="price ruled" aria-labelledby="price-title"><p class="section-label">Desktop edition / one-time</p><h2 id="price-title">Watch two folders for $19</h2><p>The free ledger imports files and exports every note. A license adds Calibre and mounted-reader folder watching on desktop.</p><div class="price-actions">${checkoutPending()}<a class="button" href="/download" data-link>Download the app</a></div><p class="fine">One-time purchase. Sociobot is the merchant of record. Core export stays free.</p></section>
  </main>${footer()}`;
}
...
function workbench(demo: boolean): string {
  ...
  if (demo && !annotations.length) {
    annotations = [...sampleAnnotations];
    saveLedger(true, annotations);
  }
  ...
  return `${header()}${demo ? demoBanner() : ""}<main id="main" class="workbench" tabindex="-1">
    ...
    <section class="desk" aria-labelledby="desk-title"><div class="transfer-rail"><h2 id="desk-title">Transfer desk</h2>
      ...
      <p id="filter-status" class="filter-status" role="status"></p><div id="ledger-list">${annotations.length ? annotationRows(annotations) : `<div class="empty"><span aria-hidden="true">¶</span><h3>No annotations yet</h3><p>Import an exported note file. Matched records will appear here.</p></div>`}</div></div></section>
  </main>${footer()}`;
}
...
function render(moveFocus = false): void {
  ...
  document
    ...
    ?.setAttribute(
      "href",
      `https://epub-annotation-bridge.sociobot.in${page === "/404" ? path : page}`,
    );
  app.innerHTML =
    page === "/"
      ? landing()
        ...
        : page === "/ledger"
          ...
          : page === "/download"
            ...
            : page === "/privacy" || page === "/terms"
              ? legal(page.slice(1) as "privacy" | "terms")
              : notFound();
  bindLinks();
  ...
  app.querySelectorAll<HTMLAnchorElement>("a[data-link]").forEach((link) =>
    link.addEventListener("click", (event) => {
      ...
      history.pushState(
        {},
        "",
        new URL(link.href).pathname + new URL(link.href).hash,
      );
      render(true);
      scrollTo(0, 0);
    }),
...
function bindWorkbench(demo: boolean): void {
  document.querySelector("#reset-demo")?.addEventListener("click", () => {
    resetDemo();
    render();
    setStatus("Demo restored to five sample highlights.");
  });
  const epubInput = document.querySelector<HTMLInputElement>("#epub-input");
  epubInput?.addEventListener("change", async () => {
    books = [...(epubInput.files ?? [])].map((file) => ({
      id: `${file.name}:${file.size}`,
      title: file.name.replace(/\.epub$/i, ""),
      author: "",
      file,
    }));
    const output = document.querySelector<HTMLOutputElement>("#book-count");
    if (output)
      output.textContent = `${books.length} EPUB${books.length === 1 ? "" : "s"} ready`;
    if (books.length)
      setStatus(
        `${books.length} EPUB${books.length === 1 ? "" : "s"} ready for quote matching.`,
      );
  });
  const annotationInput =
    document.querySelector<HTMLInputElement>("#annotation-input");
  annotationInput?.addEventListener("change", async () => {
    try {
      let incoming: Annotation[] = [];
      for (const file of [...(annotationInput.files ?? [])])
        incoming.push(...parseAnnotationFile(file.name, await file.text()));
      ...
      for (const row of incoming)
        if (!merged.some((existing) => existing.id === row.id))
          merged.push(row);
      saveLedger(demo, merged);
      render();
      setStatus(
        `Imported ${incoming.length} highlight${incoming.length === 1 ? "" : "s"}.`,
      );
    } catch (error) {
      setStatus(
        error instanceof Error
          ? error.message
          : "The notes could not be imported. Check the file and try again.",
        true,
      );
    }
  });
  document.querySelector("#export-md")?.addEventListener("click", () => {
    downloadText(
      "annotation-ledger.md",
      ledgerMarkdown(loadLedger(demo)),
      "text/markdown",
    );
  ...
  document.querySelector("#export-json")?.addEventListener("click", () => {
    downloadText(
      ...
      ledgerJson(loadLedger(demo)),
      "application/json",
    );
    setStatus("JSON exported.");
  });
  document
    .querySelector<HTMLInputElement>("#filter")
    ?.addEventListener("input", (event) => {
      const value = (
        event.currentTarget as HTMLInputElement
      ).value.toLowerCase();
      let visible = 0;
      document.querySelectorAll<HTMLElement>(".ledger-row").forEach((row) => {
      ...
      if (status)
        status.textContent = value
          ? visible
            ? `${visible} matching annotation${visible === 1 ? "" : "s"}.`
            : "No annotations match this filter."
          : "";
    });
  document
    .querySelector("#connect-folders")
    ?.addEventListener("click", connectFolders);
  document
    .querySelector("#write-sidecar")
    ?.addEventListener("click", () => void writeSidecar(demo));
}
...
async function connectFolders(): Promise<void> {
  try {
    ...
    const calibre = await open({
      directory: true,
      title: "Choose the Calibre library",
    });
    if (!calibre) {
      setStatus("No Calibre library was chosen.");
      return;
    }
    const reader = await open({
      directory: true,
      title: "Choose the mounted reader",
    });
    if (!reader) {
      setStatus("No mounted reader was chosen.");
      return;
    }
    nativePaths = { calibre: String(calibre), reader: String(reader) };
    const scan = await invoke<NativeScan>("scan_sources", {
      calibrePath: nativePaths.calibre,
      readerPath: nativePaths.reader,
    });
    const imported = importNativeScan(scan);
    await invoke("watch_paths", {
      paths: [nativePaths.calibre, nativePaths.reader],
    });
    await bindNativeWatcher();
    render();
    setStatus(
      `Connected ${scan.books.length} Calibre books and imported ${imported} highlights. Watching both folders.`,
    );
  } catch (error) {
    setStatus(
      `The folders could not be connected. ${String(error)} Choose them again.`,
      true,
    );
  }
}
...
function importNativeScan(scan: NativeScan): number {
  ...
  for (const file of scan.annotationFiles) {
    try {
      rows.push(...parseAnnotationFile(file.name, file.content));
    } catch {
      /* Ignore unrelated exports on the reader. */
    }
  }
  ...
  await listen<string[]>("source-changed", () => {
    ...
    nativeRefreshTimer = window.setTimeout(async () => {
      ...
      try {
        ...
        render();
        setStatus(
          `Folders changed. Checked ${imported} highlights and refreshed the ledger.`,
        );
      } catch (error) {
        setStatus(
          `A watched folder could not be read. ${String(error)} Reconnect both folders.`,
          true,
        );
      }
    }, 350);
...
async function writeSidecar(demo: boolean): Promise<void> {
  try {
    ...
    const directory = await open({
      directory: true,
      title: "Choose where to write the sidecar",
    });
    if (!directory) {
      setStatus("No sidecar folder was chosen.");
      return;
    }
    const path = await invoke<string>("write_sidecar", {
      directory,
      contents: ledgerJson(loadLedger(demo)),
    });
    setStatus(`New sidecar written to ${path}.`);
  } catch (error) {
    setStatus(
      `The sidecar could not be written. ${String(error)} Choose a writable folder.`,
      true,
    );
  }
}
...
function bindLicense(): void {
  document
    ...
    ?.addEventListener("submit", async (event) => {
      ...
      if (!value) {
        if (status) status.textContent = "Paste the license token first.";
        return;
      }
      storeLicense(value);
...
async function loadDownloads(): Promise<void> {
  const target = document.querySelector<HTMLElement>("#download-state");
  if (!target) return;
  ...
  try {
    const cache = JSON.parse(
      localStorage.getItem("epub-bridge:release") || "null",
    ) as { time: number; data: Release } | null;
    let data: Release;
    if (cache && Date.now() - cache.time < 3_600_000) data = cache.data;
    else {
      const response = await fetch(
        "https://api.github.com/repos/B-Divyesh/sf-epub-annotation-bridge/releases?per_page=1",
      );
    ...
    target.innerHTML = asset
      ? `<p>${escapeHtml(data.tag_name)} is ready for ${platform}.</p><a class="button primary" href="${escapeHtml(asset.browser_download_url)}">Download for ${platform}</a><a class="text-link" href="${escapeHtml(data.html_url)}">See every platform <span class="sr-only">(external)</span></a>`
      : fallback;
  } catch {
...
window.addEventListener("offline", () => {
  ...
  routeStatus.textContent =
    "You are offline. Saved pages and exports still work.";
});
window.addEventListener("online", () => {
  delete document.body.dataset.offline;
  routeStatus.textContent = "You are back online.";
});
...
if (license.checking)
  void verifyLicense().then((state) => {
    license = state;
    if (route() === "/download" || route() === "/ledger") render();
  });
