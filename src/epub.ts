...
export async function readEpub(
  ...
  BookFile & { chapters: Array<{ href: string; label: string; text: string }> }
> {
  if (file.size > 64 * 1024 * 1024)
    throw new Error(
      `${file.name} is larger than 64 MB. Choose a smaller EPUB.`,
    );
  let zip: Record<string, Uint8Array>;
  try {
    zip = unzipSync(new Uint8Array(await file.arrayBuffer()));
  } catch {
    throw new Error(`${file.name} is not a readable EPUB archive.`);
  }
  const xml = new DOMParser();
  const container = xml.parseFromString(
    strFromU8(zip["META-INF/container.xml"]),
    "application/xml",
  );
  const packagePath = container
    .querySelector("rootfile")
    ?.getAttribute("full-path");
  if (!packagePath || !zip[packagePath])
    throw new Error(`${file.name} has no readable EPUB package file.`);
  const opf = xml.parseFromString(
    strFromU8(zip[packagePath]),
    "application/xml",
  );
  const base = packagePath.split("/").slice(0, -1).join("/");
  const title =
    firstText(opf, ["dc:title", "title"]) || file.name.replace(/\.epub$/i, "");
  ...
  const chapters = [...opf.querySelectorAll("spine itemref")]
    ...
    .filter((item): item is { href: string; label: string; text: string } =>
      Boolean(item),
    );
  return { id: `${file.name}:${file.size}`, title, author, file, chapters };
}

export async function matchAnnotations(
  ...
  return annotations.map((row) => {
    ...
    const book =
      parsed.find(
        (candidate) =>
          normal(candidate.title).toLowerCase() ===
          normal(row.title).toLowerCase(),
      ) ||
      parsed.find((candidate) =>
        normal(candidate.title)
          .toLowerCase()
          .includes(normal(row.title).toLowerCase()),
      );
    if (!book) return row;
    const quote = normal(row.quote);
    const chapterIndex = book.chapters.findIndex((chapter) =>
      chapter.text.includes(quote),
    );
    if (chapterIndex < 0) return { ...row, bookId: book.id };
    const chapter = book.chapters[chapterIndex];
    const offset = Math.max(
      0,
      chapter.text.search(new RegExp(escapeRegExp(quote.slice(0, 80)), "i")),
    );
    return {
      ...row,
      bookId: book.id,
      ...
      cfi: `epubcfi(/6/${(chapterIndex + 1) * 2}!/4/1:${offset})`,
      match: "quote",
    };
  });
}
